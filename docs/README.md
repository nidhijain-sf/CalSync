# CalSync Documentation

| Document | Audience | Description |
|---|---|---|
| [Overview](./overview.md) | Everyone | What CalSync is, why it exists, what it does |
| [Installation](./installation.md) | End users | How to install on Mac and Windows |
| [Setup](./setup.md) | End users | How to connect Salesforce and Google Calendar |
| [How the Sync Works](./how-sync-works.md) | End users + curious | What gets synced, when, and how |
| [Architecture](./architecture.md) | Technical | System design, data flow, auth flows |
| [Developer Guide](./developer-guide.md) | Developers | Building, running, releasing |
| [Troubleshooting](./troubleshooting.md) | Everyone | Common issues and how to fix them |
| [Scripts & Automation](./scripts-and-automation.md) | Developers | Build scripts, GitHub Actions, labels, versioning |
| [Claude Code Commands](./claude-commands.md) | Developers | `/release`, `/test`, `/push`, `/diff`, `/summary`, `/compare`, `/log` |
